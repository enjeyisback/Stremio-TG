...
async def get_streams(media_type: str, id: str):
    ...
    import re
    streams = []
    part_files = []  # Files with partXXX pattern
    ...
    for quality in media_details.get("telegram", []):
        if quality.get("id"):
            ...
            if part_pattern.search(filename):
                part_files.append(quality)
            else:
                normal_files.append(quality)
    
    STREAM_URL = Telegram.STREAM_URL
    ...
    if part_files:
        ...
        for base_name, files in base_groups.items():
            ...
            for idx, quality in enumerate(files, 1):
                ...
                # Only show Part X/Y if there's more than one part in the group
                if total_parts > 1:
                ...
                streams.append({
                    "name": stream_name,
                    "title": stream_title,
                    "url": f"{STREAM_URL}/dl/{quality.get('id')}/video.mkv"
                })
